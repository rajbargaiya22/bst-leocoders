...
$nsc_blog_related_tax_terms = wp_get_post_terms( get_the_ID(), 'category' );
$nsc_blog_related_terms_ids = array();
foreach( $nsc_blog_related_tax_terms as $tax_term ) {
	$nsc_blog_related_terms_ids[] = $tax_term->term_id;
}

$nsc_blog_related_args['category__in'] = $nsc_blog_related_terms_ids;

if(get_theme_mod('nsc_blog_related_post',true)==1){

$nsc_blog_related_posts = new WP_Query( $nsc_blog_related_args );

if ( $nsc_blog_related_posts->have_posts() ) : ?>
    <div class="nsc-related-post">
          ...
          <?php while ( $nsc_blog_related_posts->have_posts() ) : $nsc_blog_related_posts->the_post(); ?>
            <article id="post-<?php the_ID(); ?>" <?php post_class('nsc-related-article-container'); ?>>
              ...
              <div class="">
                ...
                <?php
                if(get_theme_mod('nsc_blog_related_post_cats', true) != '0'){
                  $categories = get_the_category();
                  if ( ! empty( $categories ) ) {
                    $first_category = $categories[0]; ?>

                    <a href="<?php echo esc_attr( esc_url( get_category_link( $first_category->term_id ) ) ); ?>" class="nsc-post-cat d-inline-block mt-4 me-2" title="<?php echo esc_attr( $first_category->name ); ?>">
